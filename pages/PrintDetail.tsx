        ...
        <div className="lg:col-span-5 space-y-8">
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
                 <span className="px-3 py-1 rounded-full bg-accent-500/10 text-accent-500 text-[10px] font-bold tracking-widest uppercase border border-accent-500/20">
                    {print.category}
                 </span>
                 {print.featured && (
                    <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-amber-500/10 text-amber-500 text-[10px] font-bold tracking-widest uppercase border border-amber-500/20">
          ...
          <div className="bg-maker-900/40 backdrop-blur-sm p-6 rounded-2xl border border-white/10 shadow-xl space-y-6">
            ...
            <dl className="grid grid-cols-2 gap-x-6 gap-y-6">
                <div className="col-span-2">
                    <dt className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2">Material / Tags</dt>
                    <dd className="flex flex-wrap gap-2">
                        {print.materials.map(mat => (
                            <span key={mat} className="px-2 py-1 bg-white/5 text-slate-200 rounded text-sm font-semibold border border-white/10">
                                {mat}
                            </span>
                        ))}
                    </dd>
                ...
                {print.notes && print.notes.trim().length > 0 && (
                    <div className="col-span-2 pt-4 border-t border-white/5">
                        <dt className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-2">Maker's Notes & Constraints</dt>
                        <dd className="text-slate-300 text-sm italic leading-relaxed bg-black/20 p-4 rounded-xl border border-white/5">
                            "{print.notes}"
                        </dd>
                    </div>
                )}
